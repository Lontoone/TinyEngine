...
vec4 gaussianBlur(sampler2D tex, vec2 texSize, float radius) {
    ...
    for (float i = -radius; i <= radius; i++) {
        for (float j = -radius; j <= radius; j++) {
            gcolor += texture2D(tex, texcoord + vec2(i, j) * texelSize);
        }
    }
...
void main() {
    ...
    //Blur
    vec4 blur_color = gaussianBlur(screenTexture, u_textureSize, 1);
    ...
    vec4 color2 = gaussianBlur(screenTexture, u_textureSize, 3);
